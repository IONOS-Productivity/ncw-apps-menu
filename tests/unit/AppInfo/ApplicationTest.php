...
final class ApplicationTest extends TestCase {
	...
	private function resetOcpUtilScriptRegistries(): void {
		// Reset legacy registries - check if properties exist before accessing
		if (property_exists(\OC_Util::class, 'styles')) {
			\OC_Util::$styles = [];
		}
		if (property_exists(\OC_Util::class, 'scripts')) {
			\OC_Util::$scripts = [];
		}
	...
	public function testBootAddsExpectedScriptAndStyle(): void {
		...
		// Check styles if the property exists
		if (property_exists(\OC_Util::class, 'styles')) {
